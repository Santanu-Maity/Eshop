...
function App() {
  // Hardcoded Product List (Fallback)
  const hardcodedProducts = [
    ...
    { id: 2, name: "TV", price: 18999, image: '/img/tv.jpg', description: "42-inch Smart LED TV with HD resolution.", rating: 4 },
    { id: 3, name: "Headphone", price: 1799, image: '/img/headphone.jpg', description: "Noise-cancelling over-ear headphones.", rating: 4 },
  ...
  const [productList, setproductList] = useState(hardcodedProducts);

  const getproduct = async () => {
    try {
      const res = await fetch(`${process.env.REACT_APP_MONGO}product/all`, {
        method: "GET",
        headers: { "Content-Type": "application/json" }
      });
      const data = await res.json();

      // if API returns products, replace hardcoded
      if (data && data.length > 0) {
        setproductList(data);
      }
    } catch (error) {
      console.log("⚠️ API failed, showing hardcoded product list instead.");
    }
  };
  ...
  // store product list in localStorage
  useEffect(() => {
